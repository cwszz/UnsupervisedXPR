...
def train_model(model, train_loader):  # Train an epoch
    ...
    for step, batch in enumerate(tk):
        ...
        adv_batch_src = [tensors.to(device) for i,tensors in enumerate(batch) if i % 4 == 2]
        adv_batch_trg = [tensors.to(device) for i,tensors in enumerate(batch) if i % 4 == 3]
        with autocast():
            merge_batch_src = [torch.cat([a,b],dim=0) for a, b in zip(batch_src, adv_batch_src)]
            merge_batch_trg = [torch.cat([a,b],dim=0) for a, b in zip(batch_trg, adv_batch_trg)]

            logits1, label = model(batch_src, merge_batch_trg)
            mask = 1 - torch.cat([torch.zeros([logits1.size()[0],logits1.size()[0]]).long(), torch.eye(logits1.size()[0])], dim=1).to(device)
            logits1 = torch.mul(logits1, mask)
            loss1, acc1 = lossFunc(logits1, label)
            logits2, label = model(batch_trg, merge_batch_src)
            logits2 = torch.mul(logits2, mask)
            loss2, acc2 = lossFunc(logits2, label)
        loss = loss1 + loss2
