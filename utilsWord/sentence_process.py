...
def load_unsupervised_word_mapping(path):
    print(f'Loading words unsupervised mapping from {path}')
    f = open(path)
    en_words, lg_words = [], []
    for eachline in f.readlines():
        word1 = eachline.split('\t')[0].strip()
        word2 = eachline.split('\t')[1].strip()
        w1, w2 = word1.lower(), word2.lower()
        en_words.append((w1,w1))
        lg_words.append((w2,w2))
    return en_words, lg_words
...
class WordWithContextDatasetWW(Dataset):
    ...
    def __init__(self, words, word2context_en, word2context_de, max_len=256, model_name='xlm-roberta-base',
         prepend_bos=True, append_eos=True, sampleNum=8, mask=False, mask_percent=-1):
    ...
    def __getitem__(self, index):
        ...
        if self.sampleNum == 0:
            ...
            if self.ismasked:
                word2_id = [self.vocab.bos_token_id] + len(self.str2indices(word2)) * [self.vocab.mask_token_id] + [self.vocab.eos_token_id] 
            else:
        ...
        for each_sentence_w1,each_sentence_w2 in zip(sentences_w1,sentences_w2):
            ...
            if self.ismasked and random.randint(0,9) <= self.mask_percent:
                s_before_w2, s_word_w2, s_after_w2 = self.str2indices(each_sentence_w2[0]), len(self.str2indices(each_sentence_w2[1])) * [self.vocab.mask_token_id] \
                    , self.str2indices(each_sentence_w2[2])
            else:
